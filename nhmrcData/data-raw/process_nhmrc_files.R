...
nhmrcOutcomesGenderPostDoc <- project_grant_funded_rate_gender_new_investigator_and_teaching_load_140218_xlsx %>%
  read_excel(sheet = "AVERAGE YEARS POST DOC", col_names = FALSE, skip = 4) %>%
  remove_empty_cols() %>%
  filter(X0 != "Grand Total") %>%
  mutate(year = ifelse(grepl("^C", X0), NA, X0)) %>%
  fill(year) %>%
...
nhmrcOutcomesGenderRatesBS <- project_grant_funded_rate_gender_new_investigator_and_teaching_load_140218_xlsx %>%
  read_excel(sheet = " FUNDED RATES - BASIC SCIENCE", col_names = FALSE, skip = 3) %>%
  remove_empty_cols() %>%
  filter(X0 != "Grand Total") %>%
  mutate(year = ifelse(grepl("^C", X0), NA, X0)) %>%
  fill(year) %>%
  ...
  gather(stage, value, -ci, -year) %>%
  separate(stage, into = c("gender", "stage"), sep = "_")
save(nhmrcOutcomesGenderRatesBS, file = "data/nhmrcOutcomesGenderRatesBS.rda")

## sheet = FUNDED RATES - CLIN MED & SCI
nhmrcOutcomesGenderRatesCMS <- project_grant_funded_rate_gender_new_investigator_and_teaching_load_140218_xlsx %>%
  read_excel(sheet = " FUNDED RATES - CLIN MED & SCI ", col_names = FALSE, skip = 3) %>%
  remove_empty_cols() %>%
  filter(X0 != "Grand Total") %>%
  mutate(year = ifelse(grepl("^C", X0), NA, X0)) %>%
  fill(year) %>%
